...
(**[fresh_var ()] is a new unused variable. *)
let fresh_var () = 
  counter := !counter + 1;
...
(**[InstMap] is a collection of the type and all functionalities needed to create a map that can
   map local variables to new unused variables. *)
...
(**[instantiate xs t] replaces type t with all occurences of variables in [xs] in [t]
   replaced with new fresh variables.
   Example:
   (['a],('a -> 'b -> 'a)) is instantiated to type a1 -> 'b -> a1. *)
...
(**[generalize env t] generalizes the type t to become a Type Scheme. 
   Example: 
   type t is 'a -> 'b -> 'a. Assume that 'b is a variable that is coming into it
   from the env - this makes 'b a free variable. 
   Then generalize returns a type scheme that only captures the non free variables
   within type t.
   In this example case : (['a],('a -> 'b -> 'a)) is returned.
   View Instantiate to see what happens to this generalized type
    *)
...
(**[submap] is the type of the environment that holds all substitutions generated by
   unification. *)
...
(**[string_of_typ t] is [t] represented as a string type. *)
...
(**[infer e] is the inferred type of [e].
   Requires : [e] to be syntactically valid and type check.
   Raises : [type_err] if [e] does not meet precondition. *)
